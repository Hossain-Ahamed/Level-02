...
const createOfferedCourseIntoDB = async (payload: TOfferedCourse) => {
	...
	const isSemesterRegistationExists = await SemesterRegistrationModel.findById(semesterRegistration);
	...
	const isacademicDepartmentExists = await AcamdemicDepartmentModel.findById(academicDepartment);
	...
	const isSameOfferedCourseExistsWithSameRegisteredSemesterWithSameSection = await OfferedCourseModel.findOne({
		semesterRegistration,
		course,
		section,
	});
	...
	if (hasTimeConflict(assignedSchedules, newSchedule)) {
		throw new AppError(httpStatus.CONFLICT, `This faculty is not available at that moment! choose other time or day`);
	}
