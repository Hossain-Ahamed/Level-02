...
const updateSemesterRegistrationIntoDB = async (id: string, payload: Partial<TSemesterRegsitration>) => {
	...
	const isRegisteredSemsesterExists = await SemesterRegistrationModel.findById(id);
	...
	if (currentSemesterStatus === RegistrationStatus.ENDED) {
		throw new AppError(httpStatus.BAD_REQUEST, "This academic semester  is already ended !")
	}
	
	// upcoming --> ongoing--> ended
	if (currentSemesterStatus === RegistrationStatus.UPCOMING && payload.status === RegistrationStatus.ENDED) {
		throw new AppError(httpStatus.BAD_REQUEST, `Can not directly change from ${currentSemesterStatus} to ${requestedStatus}`)
	}
...
const deleteSemesterRegistrationsFromDB = async (id: string) => {
	...
	if(isSemesterRegistrationExist.status !== 'UPCOMING'){
		throw new AppError(httpStatus.NOT_FOUND, `Can not delete a registered semster while status is ${isSemesterRegistrationExist.status}`);
	}
	...
	const deletedOfferedCourses = await OfferedCourseModel.deleteMany({semesterRegistration : id},{session});
	const deletedRegisteredSemester = await SemesterRegistrationModel.findByIdAndDelete(id,{session});
