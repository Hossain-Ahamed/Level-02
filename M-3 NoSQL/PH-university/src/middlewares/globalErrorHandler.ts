...
export const globalErrorHandler: ErrorRequestHandler = (err, req, res, next) => {
	...
	if (err instanceof ZodError) {
		...
		({ statusCode, message, errorSources } = setErrorDetails(simplifiedError));
	  } else if (err?.code === 11000) { // duplicate key error mongoose
		const simplifiedError = handleDuplicateError(err);
		({ statusCode, message, errorSources } = setErrorDetails(simplifiedError));
	  }else if(err instanceof AppError){ // APPError handle custom
	  ...
	  }else if(err instanceof Error){ //default error pattern
		message =err?.message;
		errorSources = [{
			path : "",
			message : err.message
